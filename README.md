...
- **개인다짐 폐지 → 팀 실천약속**(운영 결정): 팀당 1건(재제출 = 수정 + 이력, 팀 정체성과 같은 규칙), 3개 카테고리 모두 필수·자유 문장 — 리더행동(리더는 ~합니다) / 팀원행동(팀원은 ~합니다) / 팀루틴·구조(우리는 ~합니다). 활동 키는 `promise`, 테이블은 `team_promises`(+`team_promise_revisions`). 현황판은 카테고리별 핵심 단어만(원문 비노출), 단어 정리 규칙은 DB 함수 `_word_stem` 하나에 모여 있다. SPEC의 R7·개인다짐 템플릿·다짐 랭킹은 더 이상 해당 없음.
- ~~개인다짐 문장은 부록 B대로 `…를 위해, 나는 ___을 하겠습니다`. 실천 내용은 명사형으로 받고(예시·도움말 변경) 을/를은 받침으로 자동 선택.~~ (폐지)
- **차수는 54개**(2026-10-12 ~ 12-04, `supabase/seed/seed_sessions.csv`). 현황판의 총 차수·'N차수의 길'은 DB의 차수 수를 따른다(40 고정 아님). 인원 분모 2,659명은 그대로.
- **차수 인원은 두 가지만**: 대상인원(`expected`)과 실참석(`actual`). 정원(`capacity`)은 팀 배정용이라 화면·집계에서 쓰지 않는다. 학습자료는 전 차수 공통이라 차수별 URL 입력란을 없앴다(`app_settings.study_default_url`).
...
- **현황판 인재상 카드 재설계**(다짐 흐름도는 개인다짐 폐지로 제거): 형용사50×명사50=2,500 조합이라 조합 Top N은 데이터의 2~17%만 설명하고 재현성이 낮음(시뮬레이션). → 다짐은 단어 기둥(형용사·명사 상위 7)+조합 리본, 인재상은 같은 단어의 Heritage↔Future 건수 비교로 변경. 조합은 3건 이상일 때만 칩으로 노출.
