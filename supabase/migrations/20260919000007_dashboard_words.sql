-- 현황판 재설계: 형용사50×명사50 조합은 2,500가지라 조합 순위만으로는 데이터의 2~17%밖에 설명하지 못하고
-- (특히 조 단위 인재상은 Top 조합이 2~6건 수준으로 우연에 가깝다) → 단어 단위 집계를 뼈대로 추가한다.
--  · pledge_flow: 형용사/명사 상위 7 + 그 사이 조합 건수(연결)
--  · hf_words:   단어별 Heritage·Future 건수(같은 어휘의 이동)
