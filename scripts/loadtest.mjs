...
// 가상 사용자 1명 = 허브 열기 → 다짐 폼 열기 → 다짐 제출 → 같은 요청 재전송(더블탭/재시도) → Pulse 폼 → Pulse 제출
...
async function discoverActions() {
  const html = await (await fetch(`${BASE}/s/${slug}/pledge`)).text();
...
const samples = { hub: [], form: [], pledge: [], retry: [], pulse: [] };
const errors = [];
...
async function virtualUser(i, ids, teams) {
  ...
  await timed("form", async () => {
    const r = await fetch(`${BASE}/s/${slug}/pledge`, { headers: { Cookie: cookie } });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    await r.text();
  });

  const pledge = { id: randomUUID(), team: { id: teams[Math.abs(i) % teams.length].id, name: "x" }, adj: "과감한", noun: "도전", action: `부하 테스트 ${i}` };
  await timed("pledge", () => action(ids.submitPledge, `/s/${slug}/pledge`, cookie, [slug, pledge]));
  await timed("retry", () => action(ids.submitPledge, `/s/${slug}/pledge`, cookie, [slug, pledge])); // 같은 uuid 재전송 = 멱등
...
try {
  const { data: s, error } = await db.from("sessions").insert({
    ...
    locks: { study: true, identity: true, finder: true, pledge: true, pulse: true },
  }).select("id").single();
  if (error) throw error;
  sessionId = s.id;
  const { data: teams } = await db.from("teams").select("id").eq("status", "active").limit(60);

  const ids = await discoverActions();
  if (!ids.submitPledge || !ids.submitPulse) throw new Error("Server Action id를 찾지 못했습니다: " + JSON.stringify(ids));
  ...
  const [pl, pu] = [await count("pledges"), await count("pulses")];
  ...
  const submitP95 = pct([...samples.pledge, ...samples.pulse], 95);
  ...
  console.log(`${pl === expected && pu === expected ? "PASS" : "FAIL"} 유실·중복 0 — 다짐 ${pl}/${expected}, Pulse ${pu}/${expected} (각자 2번씩 보냈지만 1건씩만 저장)`);
  console.log(`${submitP95 < 2000 ? "PASS" : "FAIL"} 제출 p95 ${submitP95.toFixed(0)}ms (< 2000ms)`);
} finally {
  if (sessionId) {
    for (const t of ["pledges", "pulses", "material_views", "team_identities", "finder_submissions"]) await db.from(t).delete().eq("session_id", sessionId);
