...
try {
  ...
  const ALL = { study: true, identity: true, finder: true, pledge: true, pulse: true };
  ...
  await setLocks(ALL);
  const ids = await discoverActions(BASE, ["identity", "finder", "pledge", "pulse"].map((a) => `/s/${slug}/${a}`));
  check(["submitIdentity", "submitFinder", "submitPledge", "submitPulse"].every((k) => ids[k]), "폼 4종 열림 · 제출 액션 확인");
  ...
  console.log("\n[5] 개인다짐 — 기기 기준 (R7) · 미등록 팀 (R4)");
  const pl = (cookie, action, team = { id: teams[1].id, name: "x" }) => act("submitPledge", "pledge", cookie, { id: randomUUID(), team, adj: "과감한", noun: "도전", action });
  await pl(phoneA, `${MARK} 첫 다짐`);
  await pl(phoneA, `${MARK} 고친 다짐`);
  check((await count("pledges")) === 1, "같은 기기 재제출 → 1건 유지(수정)");
  pendingName = `${MARK}TF`;
  const raw = await pl(phoneB, `${MARK} 다른 사람`, { id: null, name: pendingName });
  const { data: pend } = await db.from("teams").select("status").eq("name", pendingName).maybeSingle();
  check(raw.ok && (await count("pledges")) === 2 && pend?.status === "pending", "다른 기기 → 신규 1건 · 명부에 없는 팀은 미등록 큐로");
  const bad = await act("submitPledge", "pledge", phoneB, { id: randomUUID(), team: { id: teams[1].id, name: "x" }, adj: "과감한", noun: "도전", action: "가".repeat(81) });
  check(!bad.ok && bad.code === "invalid", "글자 수 초과(81자) → 서버 거부");
  ...
  check(dataRes.status === 200 && dash.pledges >= 2 && dash.pulse.n >= 1, "집계에 리허설 제출 반영", `다짐 ${dash.pledges} · Pulse ${dash.pulse.n}`);
  check(!dump.includes(MARK + " ") && !dump.includes(slug) && !dump.includes(teams[0].name), "현황판 데이터에 제출 원문·slug·팀명 없음");
  ...
  console.log("\n[8] 숨김 → 집계 즉시 제외 (R14)");
  const before = dash.pledges;
  await db.from("pledges").update({ hidden: true }).eq("session_id", sessionId);
  const after = (await (await get("/dashboard/data", viewer)).json()).pledges;
  check(after === before - 2, "숨긴 다짐 2건이 현황판에서 빠짐", `${before} → ${after}`);
} catch (e) {
  failed++;
  ...
  if (sessionId) {
    ...
    for (const t of ["team_identities", "finder_submissions", "pledges", "pulses", "material_views"]) await db.from(t).delete().eq("session_id", sessionId);
