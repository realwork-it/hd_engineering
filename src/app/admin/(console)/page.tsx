...
export default async function TodayPage({ searchParams }: PageProps<"/admin">) {
  ...
  const expected = sessions.reduce((a, s) => a + (s.expected ?? s.capacity ?? 0), 0);
    ...
    <>
      <div className="ad-mhead">
        <div>
          ...
          <div className="s">
            {sessions.length
              ? `${places || "장소 미정"} · ${sessions.length}개 차수${sessions.length > 1 ? " 병행" : ""} · 예상 ${expected}명${sessions.length > 1 ? " · QR은 방 이름과 색으로 구분됩니다" : ""}`
              : "이 날짜에 예정된 차수가 없습니다"}
          </div>
        </div>
        <div className="acts">
      ...
      {missingActual.length > 0 && (
        <div className="ad-alert">
          ⚠️ <span>종료된 차수 중 <b>실참석 인원 미입력</b>: {missingActual.map((s) => sessionLabel(s.display_no)).join(", ")} — 현황판 누적 인원이 예상치로 잠정 집계됩니다.</span>
        </div>
      )}
