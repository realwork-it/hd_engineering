...
import { setActual, setLock, setStatus } from "@/app/admin/(console)/actions";
...
export function TodayCard({ session: s, hubUrl, notice }: { session: SessionRow; hubUrl: string; notice: string }) {
  ...
  const [locks, setLocks] = useState<Locks>(s.locks);
  ...
  function toggle(activity: Activity) {
    ...
    setLocks((l) => ({ ...l, [activity]: open })); // 낙관적 반영
    start(async () => {
      const res = await setLock(s.id, activity, open);
      if (!res.ok) {
        setLocks((l) => ({ ...l, [activity]: !open }));
        return toast(`반영 실패: ${res.message}`);
      }
    ...
    <div className={`ad-tcard${s.status === "running" ? " running" : ""}`}>
      ...
      <div className="body">
        ...
        <div className="ad-phase">
          {s.status === "running" ? (
            <button className="ad-btn sm sec" disabled={pending} onClick={() => phase("done", "차수를 종료했습니다")}>차수 종료</button>
          ) : s.status === "done" ? (
